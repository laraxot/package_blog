...
<article class="post" itemscope itemtype="http://schema.org/Blog">
        ...
        <div class="post-inner">
            ...
            @foreach($row->content as $k=>$v)
                @component('blog::component.'.$row->content_type[$k])
                {{  $v }}
                @endcomponent
            @endforeach
            </p>
            {{--  <a class="btn btn-small btn-primary" href="{{ asset('post/' . $row->id) }}">Read More</a>
            --}}
        </div>
</article>
{{--  
...
@section('blog_sidebar')
    ...
    <div class="sidebar-box">
        ...
        <ul class="icon-list blog-category-list">
            @foreach($row->allCategories()->get() as $category)
                <li><a href="{{ $category->url }}"><i class="fa fa-angle-right"></i>{{$category->category}}</a></li>
            @endforeach
        </ul>
    </div>
